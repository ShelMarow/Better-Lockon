...
import yesman.epicfight.api.client.event.types.camera.CoupleTPSCamera;
...
//                }
            ...
            if ((playerpatch == null || !playerpatch.getEntityState().turningLocked()) &&
    ...
    private void rewroteSetupCamera(Camera camera, float partialTick, CallbackInfoReturnable<BuildCameraTransform.Pre> cir) {
        cir.cancel();
        EpicFightCameraAPI cameraAPI = (EpicFightCameraAPI) (Object) this;
        CameraAccessor cameraAccessor = (CameraAccessor) (Object) camera;
        ...
        EpicFightClientEventHooks.Camera.BUILD_TRANSFORM_PRE.post(event);
        if (event.isCanceled()) {
            cir.setReturnValue(event);
            return;
        }
        ...
        if (this.isTPSMode()) {
            ...
            float delta = ClientConfig.getTpsActivationType() == ClientConfig.TPSActivationType.ON_AIMING ? partialZoomTick / (MAX_ZOOM_TICK - 1) : 1.0F;
            ...
            cameraAccessor.betterlockon$invokeSetRotation(yRot, xRot);
            ...
            Vec3 playerPos = new Vec3(
                    ...
                    Mth.lerp(partialTick, camera.getEntity().yo, camera.getEntity().getY()) +
                            Mth.lerp((double) partialTick, cameraAccessor.betterlockon$getEyeHeightOld(), cameraAccessor.betterlockon$getEyeHeight()),
            ...
            if (Float.compare(1.0F, delta) == 0) {
                cameraAccessor.betterlockon$invokeSetPosition(nearestX, nearestY, nearestZ);
            }
            else {
                cameraAccessor.betterlockon$invokeSetRotation(this.minecraft.player.getViewYRot(partialTick), this.minecraft.player.getViewXRot(partialTick));

                cameraAccessor.betterlockon$invokeSetPosition(
                        ...
                        Mth.lerp(partialTick, this.minecraft.player.yo, this.minecraft.player.getY()) +
                                Mth.lerp(partialTick, cameraAccessor.betterlockon$getEyeHeightOld(), cameraAccessor.betterlockon$getEyeHeight()),
                        Mth.lerp(partialTick, this.minecraft.player.zo, this.minecraft.player.getZ())
                );

                cameraAccessor.betterlockon$invokeMove(-cameraAccessor.betterlockon$invokeGetMaxZoom(4.0F), 0.0F, 0.0F);
                cameraAccessor.betterlockon$invokeSetRotation(yRot, xRot);
                cameraAccessor.betterlockon$invokeSetPosition(
            ...
            if (this.minecraft.options.getCameraType() == CameraType.THIRD_PERSON_BACK) {
                ...
                cameraAccessor.betterlockon$invokeSetRotation(yRot, xRot);
                cameraAccessor.betterlockon$invokeSetPosition(finalPos.x, finalPos.y, finalPos.z);
                ...
                if (camera.isDetached()) {
                    ...
                    cameraAccessor.betterlockon$invokeSetRotation(direction != null ? direction.toYRot() - 180.0F : 0.0F, 0.0F);
                    cameraAccessor.betterlockon$invokeMove(0.0F, 0.3F, 0.0F);
                }
